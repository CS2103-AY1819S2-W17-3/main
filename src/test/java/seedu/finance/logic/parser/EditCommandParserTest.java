...
public class EditCommandParserTest {
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + CATEGORY_DESC_SHOPPING + CATEGORY_DESC_DINING
                            + CATEGORY_EMPTY, Category.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + CATEGORY_DESC_SHOPPING + CATEGORY_EMPTY
                            + CATEGORY_DESC_DINING, Category.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + CATEGORY_EMPTY + CATEGORY_DESC_SHOPPING
                            + CATEGORY_DESC_DINING, Category.MESSAGE_CONSTRAINTS);
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + AMOUNT_DESC_BOB + CATEGORY_DESC_DINING
                + DATE_DESC_AMY + NAME_DESC_AMY + CATEGORY_DESC_SHOPPING;
        ...
        EditRecordDescriptor descriptor = new EditRecordDescriptorBuilder().withName(VALID_NAME_AMY)
                ...
                .withCategories(VALID_CATEGORY_DINING, VALID_CATEGORY_SHOPPING)
                .build();
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        // categories
        userInput = targetIndex.getOneBased() + CATEGORY_DESC_SHOPPING;
        descriptor = new EditRecordDescriptorBuilder().withCategories(VALID_CATEGORY_SHOPPING).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
    ...
    public void parse_multipleRepeatedFields_acceptsLast() {
        ...
        String userInput = targetIndex.getOneBased() + AMOUNT_DESC_AMY + DATE_DESC_AMY
                + CATEGORY_DESC_SHOPPING + AMOUNT_DESC_AMY + DATE_DESC_AMY + CATEGORY_DESC_SHOPPING
                + AMOUNT_DESC_BOB + DATE_DESC_BOB + CATEGORY_DESC_DINING;

        EditRecordDescriptor descriptor = new EditRecordDescriptorBuilder().withAmount(VALID_AMOUNT_BOB)
                .withDate(VALID_DATE_BOB).withCategories(VALID_CATEGORY_SHOPPING, VALID_CATEGORY_DINING)
                .build();
        EditCommand expectedCommand = new EditCommand(targetIndex, descriptor);
